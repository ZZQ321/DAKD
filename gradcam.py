...
def show_cam_on_image(img, mask):
    ...
    mask = np.where(mask > 0.0, mask, 0)
    heatmap = cv2.applyColorMap(np.uint8(255 * mask), cv2.COLORMAP_JET)
    heatmap_ = heatmap.copy()
    # edges = cv2.Canny(heatmap, 100, 255)
     #腐蚀灰度图edges
    # kernel = cv2.getStructuringElement(cv2.MORPH_RECT,(3,3))
    #膨胀灰度图edges
    # edges = cv2.erode(edges, kernel)
    # edges = cv2.dilate(edges, kernel)
    #使得edges图像中的线条等宽平滑且清晰
    #以只检测外轮廓的检索所示寻找edges图形啊的轮廓
    # edges = cv2.GaussianBlur(edges,(3,3),0)
    # edges = cv2.blur(edges,(3,3))
    contours,hierarchy = findcontour(heatmap)
    cv2.drawContours(heatmap_, contours, -1, (0, 0, 0), 2) #哪个轮廓，-1表示所有轮廓，颜色，线条粗细
    cv2.imwrite('heatmap.jpg', heatmap)
    #将图像edges转换为三通道图像，R和G通道值都为0，B通道值为edges
    # white_edges = cv2.merge([ edges,edges, edges])
    # cv2.imwrite('edges.jpg', edges)
    heatmap = np.float32(heatmap) / 255
    # white_edges = np.float32(white_edges) / 255
    cam = heatmap + np.float32(img) 
    cam = cam / np.max(cam )
    cam = np.uint8(255 * cam)
    cv2.drawContours(cam,contours,-1,(0,0,0),2)
    cv2.drawContours(cam,contours,-1,(139,0,0),2)
    cam = np.float32(cam)/255
    # cam = np.where(white_edges>0.3,0,cam) #去除边缘位置cam的颜色
    # blue_edges = cv2.merge([edges, np.zeros_like(edges),np.zeros_like(edges)])
    # blue_edges = np.float32(blue_edges) / 255
    # cv2.imwrite('blue_edges.jpg',blue_edges)
...
class GradCam:
    def __init__(self, model, feature_module, target_layer_names, use_cuda):
        ...
        self.extractor = ModelOutputs(self.model, self.feature_module, target_layer_names) #输出最后一个layer4后的特征图1*512*7*7for res18和logits
        # ModelOutputs(model, model.layer4, ["2"])
...
def get_args():
    ...
    parser.add_argument('--image-path', type=str, default='./CAM/4.jpg',  # default='./examples/1.jpg', # './examples/both.png'
                        help='Input image path')  # default='./examples/both.png',
    parser.add_argument('--backbone', type=str, default='res18',  # default='./examples/1.jpg', # './examples/both.png'
...
if __name__ == '__main__':
    ...
    model_path = 'output/PACS/res18_real_no_mul/gamma_1.0/A/distill/A_distilled_model.pth'
    model.load_state_dict(torch.load(model_path))
    grad_cam = GradCam(model=model, feature_module=model.layer4, \
                       target_layer_names=["1"] if args.backbone=='res18' else ['2'], use_cuda=args.use_cuda)             #res50 1->2

    img = cv2.imread(args.image_path, 1)  # 读取图片文件 (H, W, BGR)
    # If set, always convert image to the 3 channel BGR color image. 
    img = np.float32(img) / 255  # 转为float32类型,范围是[0,1]
    # Opencv loads as BGR:
    # 将BGR格式的img转换为RGB格式 shape: (224, 224, 3) 即(H, W, RGB) 
    img = img[:, :, ::-1]  
