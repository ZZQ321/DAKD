...
def set_transfroms(dset, data_type, hparams, algorithm_class=None):
    ...
    if additional_data and algorithm_class is not None:  #add algorithom-specific augmentation
        for key, transform in algorithm_class.transforms.items():
            dset.transforms[key] = transform


def get_dataset(test_envs, config, algorithm_class=None):
    """Get dataset and split."""
    is_mnist = "MNIST" in config.DATA.DATASET
    ...
    for env_i, env in enumerate(dataset):
        ...
        out, in_ = split_dataset(
            env,
            int(len(env) * (1-config.DATA.SPLIT_RATE)),
            misc.seed_hash(config.SEED, env_i),
        )
        ...
        set_transfroms(in_, in_type, config.swad_kwargs, algorithm_class)   ########待续
        set_transfroms(out, out_type, config.swad_kwargs, algorithm_class)

        if config.swad_kwargs["class_balanced"]:
