...
#url download url   dst:staged path/name  than extract all name in path
def download_and_extract(url, dst, remove=True):
    gdown.download(url, dst, quiet=False)

    if dst.endswith(".tar.gz"):
        tar = tarfile.open(dst, "r:gz")
        tar.extractall(os.path.dirname(dst))# path/name -> path  
        tar.close()
...
def download_domain_net(data_dir):#download directly
    ...
    with open("data/dataclean/domain_net_duplicates.txt", "r") as f:
